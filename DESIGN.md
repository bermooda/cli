...
| Plugin/theme install | **Default: npm package name** (`npm pack` → copy into `app/plugins/<id>` or `app/themes/<id>`), merge peer deps into shop root, `npm install` in shop + in the extension dir for its own `dependencies`. Alternates: `--path` / `--git` / `--tarball`; registry fallback | Runtime discovery is filesystem + `import.meta.glob`                                                  |
...
bermooda-cli/                      # own git repo → github.com/bermooda/bermooda-cli
  ...
  .github/
    workflows/
      ci.yml
      publish.yml                  # release-please on master (OIDC npm publish)
```
...
### `bermooda plugin add <npm-package> [version]`
...
8. Merge declared peer/extra deps (`peerDependencies` + `bermooda.dependencies`) into shop root; `npm install` at shop root. Also `npm install` inside the extension directory for its own `dependencies` / `optionalDependencies` (skip both with `--skip-deps`)
...
bermooda plugin add @third-party/bermooda-plugin-foo
```
...
Until registry exists: ship `src/data/builtin-registry.json` in **this CLI repo**; support npm package names as the default install source, plus `--path` / `--git` / `--tarball`.
